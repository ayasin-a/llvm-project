...
; RUN:   -aarch64-code-layout-opt=page-cross -aarch64-code-layout-opt-page-cross-min-insns=5 \
; RUN:   -aarch64-code-layout-opt-page-cross-min-depth=2 | FileCheck %s --check-prefix=PAGECROSS
...
;------------------------------------------------------------------------------
; Page-cross alignment tests (page-cross flag of -aarch64-code-layout-opt)
;------------------------------------------------------------------------------

; Test 15: Positive - function with nested loop (depth 2) and enough instructions.
...
; Test 16: Negative - single (non-nested) loop, depth 1 below threshold of 2.
